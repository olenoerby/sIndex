"""
Seed script to populate category and tag data for the Sindex system.

This script creates a comprehensive category structure for organizing NSFW subreddits,
with each tag having 8+ keyword variants for matching.
...
from api.models import Base, Category, CategoryTag
...
def seed_categories():
    """Seed the database with category and tag data."""
    ...
    with Session(engine) as session:
        ...
        for category_name, category_data in CATEGORY_DATA.items():
            ...
            for tag_data in category_data.get("tags", []):
                ...
                existing_tag = session.query(CategoryTag).filter(
                    CategoryTag.category_id == category.id,
                    CategoryTag.name == tag_name
                ).first()
                
                if existing_tag:
                    print(f"  ℹ️  Tag '{tag_name}' already exists, skipping...")
                    continue
                ...
                session.add(tag)
                tags_created += 1
                print(f"  🏷️  Created tag: {tag_name} ({len(keywords.split(','))} keywords)")
